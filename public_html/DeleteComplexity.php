...
-->
<head>
    <title>Delete Economic Complexity</title>
    <meta charset="utf-8">
...
<body>
    ...
    } else if (!is_numeric($year) || is_numeric($country)) {
        echo "<h2>Year must be a number, country must be a string</h2>";
    } else {
        if ($stmt = $conn->prepare("DELETE FROM Economic_Complexity WHERE country_name LIKE ? AND year = ?")) {
